...
class ANFIS:
    ...
    def __init__(self, X, Y, memFunction):
        ...
        self.memFuncsByVariable = [[x for x in range(len(self.memFuncs[z]))]
    ...
    def _clip_mf_params(self,
                        mean_min=0.0, mean_max=1.0,
                        sigma_min=0.05, sigma_max=0.4):
        """
        Restringe los parámetros de las funciones de pertenencia
        para evitar MFs degeneradas (picos infinitos o casi constantes).
        Se asume que las entradas están normalizadas en [0, 1].
        """
        for var in range(len(self.memFuncs)):          # cada variable de entrada
            for mf in range(len(self.memFuncs[var])):  # cada MF de esa variable
                params = self.memFuncs[var][mf][1]     # diccionario {'mean': .., 'sigma': ..}

                if 'mean' in params:
                    if params['mean'] < mean_min:
                        params['mean'] = mean_min
                    elif params['mean'] > mean_max:
                        params['mean'] = mean_max

                if 'sigma' in params:
                    if params['sigma'] < sigma_min:
                        params['sigma'] = sigma_min
                    elif params['sigma'] > sigma_max:
                        params['sigma'] = sigma_max
    ...
    def trainHybridJangOffLine(self, epochs=5, tolerance=1e-5, initialGamma=1000, k=0.01):
        ...
        while (epoch < epochs) and (convergence is not True):
            ...
            # limitar parámetros de las MF después de cada actualización
            self._clip_mf_params()
